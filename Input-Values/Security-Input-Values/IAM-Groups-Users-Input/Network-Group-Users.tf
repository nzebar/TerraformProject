...
variable "NETWORK_GROUPS_USERS" {
  ...
  default     = {
    tempalate = {
    ##### Group Settings #####
        "name_group" = ["TestGroup2"]
        "path_group" = ["/this/is/a/test/group/path/"]
        ## Group Policy Settings ##
        "group_policy_name" = ["TestGroupPolicyName2"]
    ...
    ##### Group Memebership Settings #####
      "membership_name" = ["testGroupmembName"]
        ...
        "users_with_console_access" = ["user4", "user5", "user6"]
        ...
        "create_access_keys" = {
            "user4" = "keybase:scrumlord",
            "user5" = "keybase:scrumlord",
            "user6" = "keybase:scrumlord"
        }
...
module "NETWORK_GROUPS_USERS" {
for_each = var.NETWORK_GROUPS_USERS

source = "../../../Modules/Security-Modules/IAM-Modules/Create-Groups-Module"
    # Name of IAM Group & IAM Group Policy

        name_group = lookup(var.NETWORK_GROUPS_USERS[each.key], "name_group" == [""] ? null : "name_group", null)

        path_group = lookup(var.NETWORK_GROUPS_USERS[each.key], "path_group", [""])

        membership_name = lookup(var.NETWORK_GROUPS_USERS[each.key], "membership_name", [""])

        group_policy_name = lookup(var.NETWORK_GROUPS_USERS[each.key], "group_policy_name", [""])

        put_path_aws_group_policy = lookup(var.NETWORK_GROUPS_USERS[each.key], "put_path_aws_group_policy", [""])

        group_policy_local_path = lookup(var.NETWORK_GROUPS_USERS[each.key], "group_policy_local_path", [])
    ...
    # Can specify users in the AWS console or users created through the "Create-User" module.
        ...
        add_existing_console_users = lookup(var.NETWORK_GROUPS_USERS[each.key], "add_existing_console_users" == [""] ? null : "add_existing_console_users", [""])

        users_with_console_access = lookup(var.NETWORK_GROUPS_USERS[each.key], "users_with_console_access", [""])

        password_reset_required = lookup(var.NETWORK_GROUPS_USERS[each.key], "password_reset_required", ["true"])

        put_path_console_users = lookup(var.NETWORK_GROUPS_USERS[each.key], "put_path_console_users", [""])

        create_access_keys = lookup(var.NETWORK_GROUPS_USERS[each.key], "create_access_keys", [{}])

        force_destroy = lookup(var.NETWORK_GROUPS_USERS[each.key], "force_destroy", ["false"])
    } 
